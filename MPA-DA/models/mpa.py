...
class ResNet(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # print("coarse_y.size(): ", coarse_y.size())
        # print("torch.unique(coarse_y): ", torch.unique(coarse_y))
        proj_key_y = features_y.view(b, C, -1).permute(0, 2, 1)
        proj_query_y = torch.sigmoid(coarse_y).view(b, 1, -1)
        # print("torch.unique(proj_query_y): ", torch.unique(proj_query_y))
