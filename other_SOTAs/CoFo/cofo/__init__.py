...
class CoFoLearner(Learner):
    def fit(self, n_epochs, optim, lr_scheduler, cofo_loss, beta=0.09, contrastive=True, alpha=1.0, alpha_content=1.0,
        ...
        # By hand validation, so I can use the results directly for the metrics
        ...
        seg_metric = Dice().to("cuda")
        seg_metric_IoU = JaccardIndex(task="binary").to("cuda")
        ...
        for epoch in bar:
            ...
            if epoch % validate_every == 0:
                ...
                log += f'dice_score_on_target_test: {seg_metric.compute()}|'
                log += f'IoU_score_on_target_test: {seg_metric_IoU.compute()}|'

                seg_metric.reset()
                seg_metric_IoU.reset()
